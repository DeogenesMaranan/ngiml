...
def predict_probability_map(
    ...
    with torch.no_grad():
        with torch.autocast(device_type="cuda", dtype=autocast_dtype or torch.float16, enabled=use_amp):
            ...
            logits = select_highest_resolution_head(outputs)
    ...
    def _flush(
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=autocast_dtype or torch.float16, enabled=use_amp):
                ...
                logits = select_highest_resolution_head(outputs) if isinstance(outputs, (list, tuple)) else outputs
        ...
        records.append(
            {
                ...
                "checkpoint_epoch_from_name": parse_checkpoint_epoch(checkpoint_path),
